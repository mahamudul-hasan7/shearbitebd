...
export default function RoleSelectionPage() {
  return (
    <AuthShell eyebrow="Join the ShareBite BD community" title="Choose the role that matches your work." description="Each role receives a focused dashboard, permissions, and workflow designed for safe food rescue coordination.">
      <Link href="/onboarding" className="mb-6 inline-flex items-center gap-2 text-sm font-black text-brand-700 hover:text-brand-800"><ArrowLeft className="size-4" /> Back</Link>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        <RoleCard icon={UtensilsCrossed} title="Food Donor" description="Post surplus food, coordinate pickup, and track impact." href="/register?role=donor" />
        <RoleCard icon={Building2} title="NGO / Organization" description="Discover, claim, receive, and distribute rescued food." href="/register?role=ngo" tone="blue" />
        <RoleCard icon={UsersRound} title="Volunteer" description="Support verified pickup and delivery tasks nearby." href="/register?role=volunteer" tone="accent" badge="Future pilot" />
      ...
      <p className="mt-7 text-center text-sm text-muted-600">Already have an account? <Link href="/login" className="font-black text-brand-700 hover:text-brand-800">Log in</Link></p>
    </AuthShell>
  );
}
