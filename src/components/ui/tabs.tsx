...
export function Tabs({ items }: { items: Array<{ label: string; href: string; active?: boolean; count?: number }> }) {
  return (
    <div className="flex gap-2 overflow-x-auto rounded-2xl border border-line bg-white p-1.5 shadow-sm">
      {items.map((item) => (
        <Link
          key={item.label}
          href={item.href}
